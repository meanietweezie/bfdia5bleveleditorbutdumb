                        ...
                        if(numberThing) {
                            ...
                            if(ypos > levelDataObj[idthig].levelData.length) {
                                ...
                                try{
                                    ...
                                    fs.writeFileSync(levelPath,JSON.stringify(levelDataObj[idthig]))   
                                    ...
                                    fs.writeFileSync(cooldownPath,JSON.stringify(cooldownObj[idthig]))
                                }
            ...
            let timeLeft = (cooldownObj[interaction.guildId].name != undefined) ? Math.floor(Date.now() / 1000) - cooldownObj[interaction.guildId].name : 0
                if(timeLeft < 0 && !checkPerms(interaction.member,interaction)) {
                    ...
                    if(message.length < 1 || message == "" || !/^[\u0020-\u007e\u00a0-\u00ff‘’]*$/.test(message)) {
                        ...
                        fs.writeFileSync(levelPath,JSON.stringify(levelDataObj[interaction.guildId]))  
                        ...
                        fs.writeFileSync(cooldownPath,JSON.stringify(cooldownObj[interaction.guildId]))
                    }
            ...
            case "confirmrow":
                interaction.deferReply({flags:MessageFlags.Ephemeral}).then(() => {
                    ...
                    if(rowNow[0] == "row") {
                        ...
                        fs.writeFileSync(levelPath,JSON.stringify(levelDataObj[rowNow[2]]))
